...
filename="'trtype=RDMA adrfam=IPv4 traddr=192.168.2.110 trsvcid=4421 ns=1'" #####！！！error
# numjob: 
numjobs=(1)
...
do_test() {
  ...
  echo "LD_PRELOAD=/home/lwg/spdk/myfiles/fio_plugin fio -rw=${rw_type} -bs=${bs}k -size=${size} -runtime=${runtime} -iodepth=${iodepth} \
         ...
         --ramp_time=${ramp_time} --rate_iops=${rate_iops} --output ${output} --output-format=json '--filename=trtype=PCIe traddr=0000.08.00.0 ns=1'"
...
# beginto test 1 numjobs; 1 depth (randread randwrite)
